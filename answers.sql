...
SELECT COUNT(*) AS count FROM models WHERE brand_name = 'Ford';
...
SELECT models.name, brand_name, brands.founded FROM models LEFT JOIN brands ON brands.name=models.brand_name WHERE year = 1960;
...
SELECT brand_name from models JOIN brands ON brand_name=brands.name GROUP BY brand_name HAVING COUNT(*) > 5;
...
SELECT name, year FROM models WHERE year IN (SELECT founded FROM brands GROUP BY founded);